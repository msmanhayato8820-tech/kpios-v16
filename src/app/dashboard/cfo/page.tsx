...
import {
  ...
  ResponsiveContainer, Legend, ComposedChart, Cell,
  PieChart, Pie, AreaChart, Area,
} from 'recharts';
import { CFO_KPIS, ARR_HISTORY, MONTHLY_FINANCE, PAYMENT_ALERTS, COST_BREAKDOWN } from '@/data/mock';
import { Kpi, PaymentStatus } from '@/types';
import KpiCard from '@/components/KpiCard';

type Tab = 'kpi' | 'revenue' | 'cost' | 'payments';
...
export default function CfoDashboard() {
  ...
  // KPI分類
  const riskKpis = CFO_KPIS.filter((k) => k.status === 'risk');
  const otherKpis = CFO_KPIS.filter((k) => k.status !== 'risk');
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div>
        <h1 className="text-2xl font-bold text-[var(--text-primary)]">CFO Dashboard</h1>
        <p className="text-sm text-[var(--text-tertiary)] mt-0.5">
          ファイナンス・キャッシュフロー・入金管理
      ...
      {/* Overdue Alert (always visible if overdue exists) */}
      {overdueTotal > 0 && (
        <div className="rounded-xl p-3 border border-red-500/30 bg-red-500/5 cursor-pointer hover:bg-red-500/10 transition-all"
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <span className="text-xs font-semibold text-red-400">
                延滞 {PAYMENT_ALERTS.filter(p => p.status === 'overdue').length}件・{overdueTotal.toLocaleString()}万円
              </span>
              <span className="text-[10px] text-red-300/60 ml-2">クリックで詳細</span>
            </div>
          </div>
        </div>
      )}
      ...
      <div className="flex items-center gap-1 border-b border-[var(--border)]">
        {([
          ...
          { id: 'revenue' as const, label: '売上・利益' },
          { id: 'cost' as const, label: 'コスト分析' },
          { id: 'payments' as const, label: '入金管理', badge: PAYMENT_ALERTS.filter(p => p.status === 'overdue').length },
        ]).map(t => (
      ...
      {tab === 'revenue' && (
        <>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            ...
            <SummaryCard
              label="通期利益見込み"
              value={`${(annualProfitForecast / 10000).toFixed(1)}億円`}
              sub={`計画比 ${((annualProfitForecast / annualProfitPlan) * 100).toFixed(0)}%`}
              highlight={annualProfitForecast >= annualProfitPlan ? 'green' : 'red'}
            />
          </div>
          ...
          {/* Profit Chart */}
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] shadow-[var(--shadow-card)] p-5">
            <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-4">月次営業利益（百万円）</h3>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
          ...
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] shadow-[var(--shadow-card)] p-5">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-[var(--border)]">
                    ...
                    <th className="text-right py-2 px-2 text-[var(--text-tertiary)] font-medium">コスト</th>
              ...
              <p className="text-[10px] text-[var(--text-tertiary)] mt-2">* 紫字 = 見込み値（未確定）</p>
            </div>
          </div>
        </>
      )}
      ...
      {tab === 'cost' && (
        <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            {/* Cost Pie */}
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] shadow-[var(--shadow-card)] p-5">
              <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-4">コスト構成比（直近月）</h3>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie data={costPieData} cx="50%" cy="50%" innerRadius={60} outerRadius={100}
            ...
            {/* Cost Bar */}
            <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] shadow-[var(--shadow-card)] p-5">
              <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-4">計画 vs 実績（万円）</h3>
              <div className="h-72">
                <ResponsiveContainer width="100%" height="100%">
          ...
          {/* Cost Table */}
          <div className="rounded-xl border border-[var(--border)] bg-[var(--card-bg)] shadow-[var(--shadow-card)] p-5">
            <h3 className="text-sm font-medium text-[var(--text-secondary)] mb-3">コスト明細</h3>
            <div className="overflow-x-auto">
              <table className="w-full text-xs">
                <thead>
                  <tr className="border-b border-[var(--border)]">
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-300/70 mt-0.5 leading-relaxed">
                  現在はモックデータで動作中。本番ではMoneyForward クラウド請求書APIと連携し、リアルタイムの入金状況・売上データを自動取得する設計です。
                </p>
              </div>
            </div>
          </div>
